...
// ztj added ,用于navigation构建的时候使用的参数配置数据
...
struct rcConfig
{
	...
	// Q ztj added
	// 障碍物和可行走高度区域之间的距离，这个和borderSize是什么关系呢?
	...
	// regions中的nspan小于这个值 会合并到大区域
	/// Any regions with a span count smaller than this value will, if possible, 
	/// be merged with larger regions. [Limit: >=0] [Units: vx] 
	int mergeRegionArea;
	
	// 登高面到多边形转变时，每个多边形可使用的最大顶点数
...
struct rcCompactHeightfield
{
	...
	float bmax[3];				///< The maximum bounds in world space. [(x, y, z)]
	// Q ztj added
	// cs,ch 和 width height有什么关系
	// widht height 是整个heightFiled的长和宽
	// cs,ch 是cell的宽,和cell的高。
	// wdith height 是个长方体
	// 按照cs,ch切分程小的长方体，细分width,height这个大的长方体
	float cs;					///< The size of each cell. (On the xz-plane.)
	float ch;					///< The height of each cell. (The minimum increment along the y-axis.)
...
struct rcContour
{
	...
	// Q ztj added
	// Raw contour 是什么意思
	// contour是等高线，相同平面上的4个点区域确定了此等高线？？？
	// y轴上的切片吗?
...
struct rcPolyMesh
{
	...
	// Q ztj added
	// flags 是什么东西做什么的滴
	// areas 是指 polyMesh可以在多个区域中码,area和上面的regs有什么区别呢，area和region

...
{
	// Q ztj added 
	// 将y值一样的ch大小的块压缩成长条~~~~ cs x ch*cnt  宽高大小
	// cell 生成，xz平面上的cs状分层体。最大MAX_LAYERS-1层
	rcAssert(ctx);
			...
			for (int i = (int)c.index, ni = (int)(c.index+c.count); i < ni; ++i)
			{
				...
				for (int dir = 0; dir < 4; ++dir)
				{
					...
					for (int k = (int)nc.index, nk = (int)(nc.index+nc.count); k < nk; ++k)
					{
						...
						// walkableHeight y轴上高度，>= walkableHeight表示竖直高度可以通过人
						// walkableClimb z轴上间隙宽度, <=walkableClimb表示水平距离可以跨过去
						if ((top - bot) >= walkableHeight && rcAbs((int)ns.y - (int)s.y) <= walkableClimb)
						{
							...
							{
								// Q ztj added
								// 这个表示，MAX_LAYERS高度个ch的不计算在内 why????
								// 从底部噜到顶不更好吗???
